...
## Keep(北京卡路里体育有限公司) （ 2018年8月 ~ 2019-12月 ）

### KeepLand
- 负责 Keepland 线下运动空间播放教学系统的设计、开发、维护、运维及门店智能化改造 (蓝牙网关)
...
- 为提高播放系统稳定性，完善系统架构。本地时钟模块、SSH 服务、日志服务、故障自动恢复模块、心跳服务、缓存模块、命令分发模块，使播放系统稳定性从 97.50% 提高到 99.70%。通过重启可以解决所有播放端问题

### 播放系统安装管家
- 人工仅需操作3步，即可完成电视盒子的 root，播放系统的安装、设置等操作
...
- 为节省测试资源，搭建 UI 自动化测试平台(使用 Espresso 编写UI测试用例，Spoon管理、生成友好的测试结果并结合 Jenkins 持续集成)
...
- 增强了Photoshop 拼图的能力，学会了 Illustrator 的使用
